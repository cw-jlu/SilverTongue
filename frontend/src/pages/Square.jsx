...
import { MessageSquare, ThumbsUp, Search, Send, Sparkles, User, ShieldAlert } from 'lucide-react';
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <h2 style={{ fontSize: '24px', fontWeight: 'bold', color: '#111827', margin: '0 0 4px' }}>🏘 学习社区广场</h2>
          <p style={{ fontSize: '14px', color: '#6b7280', margin: 0 }}>与全球语言学习者分享你的心得、语法纠错和对练故事</p>
        </div>
        <div style={{
          padding: '6px 12px',
      ...
      <div className="card" style={{
        ...
        <textarea
          placeholder="今天学到了什么好表达？或者跟 AI 外教聊了些什么？分享一下吧..."
          value={content}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        {posts.length === 0 ? (
          <div style={{
            ...
            <p style={{ margin: 0, fontSize: '14px' }}>暂无相关帖子，快来发表第一条动态吧！</p>
          </div>
        ) : (
          posts.map(p => {
            // 生成炫彩头像背景色
            const colors = ['#f43f5e', '#ec4899', '#d946ef', '#a855f7', '#8b5cf6', '#6366f1', '#3b82f6', '#0ea5e9'];
            const charCodeSum = (p.nickname || p.username || 'U').split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
              ...
              <div key={p.id} className="card" style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div style={{
                    ...
                    {(p.nickname || p.username || 'U').substring(0, 1).toUpperCase()}
                  </div>

                  <div>
                    <div style={{ fontSize: '14px', fontWeight: 'bold', color: '#111827' }}>
                      {p.nickname || p.username || '匿名用户'}
                    </div>
